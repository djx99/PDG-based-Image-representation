...
class GaussianImage_Covariance(nn.Module):
    ...
    def get_gaussian_params_for_viz(self):
        """Return gaussian params on CPU for visualization."""
        with torch.no_grad():
            means = self.xys.detach().float().cpu()  # [N,2] in pixels
            cov2d = self.get_cov2d_elements.detach().float().cpu()  # [N,1]
        return means, cov2d

    def forward_for_superresolution(
            self,
            scale: float,
            isprint: bool = False,
    ):
        ...
        H = int(round(self.H * scale))
        W = int(round(self.W * scale))
        ...
        # 1) scale-aware transform
        xyz = self.get_xyz
        cov = self.get_cov2d_elements  # [N,3] cholesky elements (Lxx, Lxy, Lyy) in your convention

        xyz_sr = (xyz + 0.5) * scale - 0.5
        cov_sr = cov * scale * scale

        # 2) project + rasterize on larger canvas
        xys, depths, radii, conics, num_tiles_hit = project_gaussians_2d_covariance(
            xyz_sr, cov_sr,
            H, W, tile_bounds,
            coords_norm=self.coords_norm,
            clip_coe=self.gs_clip_coe,
        ...
        out_img = rasterize_gaussians_plus(
        ...
        out_img = torch.clamp(out_img, 0.0, 1.0)  # [H,W,3]
        out_img = out_img.view(-1, H, W, 3).permute(0, 3, 1, 2).contiguous()  # [B,3,H,W]

        out_img = out_img[:, :, (int)(H*0.25):(int)(H*0.45), (int)(W*0.45):(int)(W*0.6), ]
