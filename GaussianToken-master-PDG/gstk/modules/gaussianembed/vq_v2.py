...
def batched_sample_vectors(trainer, samples, num):
...
def all_gather_sizes(trainer, x, dim):
    size = torch.tensor(x.shape[dim], dtype=torch.long, device=x.device)
    # 自动收集所有进程的 size 张量，无需手动创建 all_sizes 列表
    all_sizes = trainer.strategy.all_gather(size)
    return all_sizes

def all_gather_variably_sized(trainer, x, sizes, dim=0):
    rank = trainer.global_rank
    all_x = []

    # 对于每个 size 和 rank，使用 Lightning 的 broadcast
    for i, size in enumerate(sizes):
        t = x if i == rank else x.new_empty(pad_shape(x.shape, size, dim))

        t = trainer.strategy.broadcast(t, src=i).to(x.device)
        all_x.append(t)

    trainer.strategy.barrier()
    return all_x

def sample_vectors_distributed(trainer, local_samples, num):
    local_samples = rearrange(local_samples, '1 ... -> ...')

    rank = trainer.global_rank
    all_num_samples = all_gather_sizes(trainer, local_samples, dim=0)

    if rank == 0:
        # 如果当前进程是 rank 0，采样分配给各 rank 的数量
        samples_per_rank = sample_multinomial(num, all_num_samples / all_num_samples.sum())
    else:
        samples_per_rank = torch.empty_like(all_num_samples)

    # 使用 Lightning 的 broadcast
    samples_per_rank = trainer.strategy.broadcast(samples_per_rank, src=0)
    samples_per_rank = samples_per_rank.tolist()

    # 采样和聚合每个 rank 的样本
    local_samples = sample_vectors(local_samples, samples_per_rank[rank])
    all_samples = all_gather_variably_sized(trainer, local_samples, samples_per_rank, dim=0)
...
def kmeans(
    ...
    means = sample_fn(trainer, samples, num_clusters)
...
@cache # 避免重复计算
def is_distributed(trainer):
    return trainer.is_distributed
...
class EuclideanCodebook(Module):
    ...
    @torch.jit.ignore
    def init_embed_(self, trainer, all_reduce_fn, data, mask = None):
        if self.initted:
            return
    ...
    def replace(self, trainer, batch_samples, batch_mask):
        for ind, (samples, mask) in enumerate(zip(batch_samples, batch_mask)):
            sampled = self.replace_sample_fn(trainer, rearrange(samples, '... -> 1 ...'), mask.sum().item())
    ...
    def expire_codes_(self, trainer, batch_samples):
        ...
        self.replace(trainer, batch_samples, batch_mask = expired_codes)
    ...
    def forward(
        ...
        all_reduce_fn = lambda x, op='sum': trainer.strategy.reduce(x, reduce_op=op) if trainer else noop
        ...
        if self.training:
            self.init_embed_(trainer, all_reduce_fn, flatten, mask = mask)
...
class VectorQuantize(Module):
    ...
    def forward(
        ...
        quantize, embed_ind, distances = self._codebook(trainer, x, **codebook_forward_kwargs)
