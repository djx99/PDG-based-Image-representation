...
class GaussianRefinementModule(nn.Module):
    def __init__(
        ...
        self.opa_dim = 3 if include_opa else 0
        
        # self.output_dim = 5 + int(include_opa) + dim_feature # 2 + 1 + 2 + 1 + dim_feature
        # self.feature_start = 5 + int(include_opa)
        self.output_dim = 5 + self.opa_dim + dim_feature # 2 + 1 + 2 + 1 + dim_feature
        self.feature_start = 5 + self.opa_dim
    ...
    def get_gaussian(self, output):
        ...
        if self.include_opa:
            xys = safe_sigmoid(output[..., :2]) # (0, 1)
            rots = output[..., 2:3]
            opacities = output[..., 5: (5 + self.opa_dim)]
            gs_scales = output[..., 3:5]
            ...
            xys = _apply_range(xys, self.xy_range)  # (-1, 1)
            rots = safe_sigmoid(output[..., 2:3]) * torch.pi  # (0, pi)
            opacities = safe_sigmoid(output[..., 5: (5 + int(self.include_opa))])  # (0, 1)

            gs_scales = safe_sigmoid(output[..., 3:5]) # [0, 1]
            gs_scales = _apply_range(gs_scales, self.range_scheduler.current_range)
            if self.training:
                self.range_scheduler.update(gs_scales.mean().item())  # (0.1, 4.0) -> (0.1, 3.0)
