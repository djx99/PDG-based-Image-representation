...
class GaussianLifter(nn.Module):
    def __init__(
        ...
        xy = safe_inverse_sigmoid(xy) # (-inf, inf) sigmoid 的反函数（logit）把一个(0,1)范围内的数映射到整个实数域
        ...
        if include_opa:
            ...
            opacity = torch.zeros(num_anchor, 3, dtype=torch.float)
        else:
...
class AnchorEncoder(nn.Module):
    ...
    def __init__(
        ...
        if include_opa:
            ...
            self.opacity_fc = embedding_layer(3)
    ...
    def forward(self, anchor: torch.Tensor, **kwargs):
        ...
        if self.include_opa:
            opacity_feat = self.opacity_fc(anchor[..., 5:5+self.opa_dim])
        else:
            opacity_feat = 0.
