...
class GQGAN(L.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if batch_idx == 0 and getattr(self, "global_rank", 0) == 0:
            print("[DEBUG] x.shape =", tuple(x.shape),
                  "min/max =", float(x.min()), float(x.max()))
        # ===========================================

        xrec, indices, loss_commit, gaussians = self(x)

        # ===== [新增] 打印 tokens/indices 的形状（通常就能判断是不是 8×8）=====
        if batch_idx == 0 and getattr(self, "global_rank", 0) == 0:
            if indices is None:
                print("[DEBUG] indices is None")
            elif isinstance(indices, (list, tuple)):
                print("[DEBUG] indices is list/tuple:", [getattr(t, "shape", type(t)) for t in indices])
            else:
                print("[DEBUG] indices.shape =", tuple(indices.shape))
        # ======================================================================
