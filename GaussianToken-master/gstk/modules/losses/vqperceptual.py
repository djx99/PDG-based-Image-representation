...
def update_weight_threshold(weight, epoch, threshold=0, value=0.):
    if epoch < threshold:
        weight = value
    return weight
...
class VQLPIPSWithDiscriminator(nn.Module):
    def __init__(self, disc_start_epoch, gan_start_epoch, pixelloss_weight=1.0, disc_num_layers=3, disc_in_channels=3, disc_factor=1.0, disc_weight=1.0,
                 commit_weight = 0.25, perceptual_weight=1.0, use_actnorm=False, disc_conditional=False,
        ...
        self.discriminator_start_epoch = disc_start_epoch
        self.gan_start_epoch = gan_start_epoch
    ...
    def forward(self, loss_commit, inputs, reconstructions, optimizer_idx,
                epoch, last_layer=None, cond=None, split="train"):
        ...
        if optimizer_idx == 0:
            ...
            disc_factor = update_weight_threshold(self.disc_factor, epoch, threshold=self.gan_start_epoch)

            if not self.training:
                real_g_loss = disc_factor * g_loss
            g_loss = d_weight * disc_factor * g_loss

            loss = nll_loss + g_loss + loss_commit * self.commit_weight
            if disc_factor == 0:
                log = {"{}/total_loss".format(split): loss.clone().detach(),
                       "{}/commit_loss".format(split): loss_commit.detach(),
        ...
        if optimizer_idx == 1:
            ...
            if cond is None:
                logits_real = self.discriminator(inputs.contiguous().detach()) # TODO: detach to avoid double grad
                logits_fake = self.discriminator(reconstructions.contiguous().detach())
            else:
                logits_real = self.discriminator(torch.cat((inputs.contiguous().detach(), cond), dim=1))
                logits_fake = self.discriminator(torch.cat((reconstructions.contiguous().detach(), cond), dim=1))

            disc_factor = update_weight_threshold(self.disc_factor, epoch, threshold=self.discriminator_start_epoch)
            #---------------------------------------------------------------------------------------
